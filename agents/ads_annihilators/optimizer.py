...
class Optimizer:
    ...
    def _make_prices_to_predict(self):
        ...
        p0_to_predict_comb = np.concatenate([
            np.linspace(self.min_p0, self.max_p0_b, 150),
            np.linspace(self.max_p0_b + 0.1, self.max_p0 / 3, 50),
            np.linspace(self.max_p0 / 3 + 0.1, self.max_p0, 50)
        ])
        p1_to_predict_comb = np.concatenate([
            np.linspace(self.min_p1, self.max_p1_b, 200),
            np.linspace(self.max_p1_b + 0.1, self.max_p1 / 3, 75),
            np.linspace(self.max_p0 / 3 + 0.1, self.max_p1, 75)
        ])
